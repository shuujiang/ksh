...
(function(j) {
	var p = j.utils, c = {}, s, a = {}, i = null, n = {}, d = false, b = ".jwplayer ";
	function q(u) {
	...
	p.hexToRgba = function(w, v) {
		var x = "rgb";
		var u = [ parseInt(w.substr(1, 2), 16), parseInt(w.substr(3, 2), 16),
			...
			if (F >= 0) {
				...
				if (F - O > 0) {
					...
					if (!J) {
						...
						L.fillText(I + "     ", K, P + 4);
						L.drawImage(s, t.width
								- ((t.width - L.measureText(I).width) / 2) - 4,
								(k - s.height) / 2);
						if (n.isMobile()) {
							var M = new n.touch(o);
...
(function(e) {
	var h = e.html5, m = e.utils, o = e.events, p = o.state, n = e.parsers, k = m.css, j = "playing", l = document, a = ".jwcaptions", b = "absolute", c = "none", i = "100%", f = "hidden", g = "normal", d = "#FFFFFF";
	h.captions = function(w, O) {
		var u = w, aa, v = {
			...
			fontWeight : g,
			textDecoration : c
		}, z, q, L, x = [], ab = 0, Q = -1, T = 0, N = false, K, U = new o.eventdispatcher(), s = m
				.isAndroid(4)
				&& !m.isChrome(), t = this;
		m.extend(this, U);
		function Y() {
		...
		function X() {
			var ac = aa.offsetHeight, ad = aa.offsetWidth;
			if (ac !== 0 && ad !== 0) {
		...
		function C() {
			...
			var ak = u.jwGetPlaylist()[u.jwGetPlaylistIndex()], ah = ak.tracks, ag = [], af = 0, ai = "", ac = 0, ad = "", aj;
		...
		function S(ad, ac) {
			var ah = ad.responseXML ? ad.responseXML.firstChild : null, ag;
			ab++;
			if (ah) {
...
(function(k) {
	var w = k.html5, J = k.utils, m = k.events, D = m.state, B = J.css, p = J.transitionStyle, d = J
			.isMobile(), c = J.isAndroid(4) && !J.isChrome(), H = "button", f = "text", l = "divider", v = "slider", t = "relative", a = "absolute", E = "none", i = "block", h = "inline", G = "inline-block", g = "hidden", s = "left", y = "right", b = "100%", r = "opacity .25s, background .25s, visibility .25s", z = 250, F = {
		display : E
	}, C = {
		display : i
	}, o = {
		display : u
	}, A = "span.jwcontrolbar", x = "array", I = false, n = true, q = null, u, j = window, e = document;
	w.controlbar = function(aE, aP) {
		var b0, b1, bO = a2("divider", l), aS = {
			margin : 8,
			maxwidth : 800,
			...
			layout : {
				left : {
					position : "left",
					elements : [ a2("play", H), a2("prev", H), a2("next", H),
							a2("elapsed", f) ]
				},
				center : {
					position : "center",
				...
				right : {
					position : "right",
					elements : [ a2("duration", f), a2("hd", H), a2("cc", H),
							a2("mute", H), a2("volume", v), a2("volumeH", v),
							a2("fullscreen", H) ]
				}
			}
		}, bf, aF, aR, bL, aC, b3, bT, bc, aM, aj, bz, b4, bd, aW, av, bt, S, bA, a0, bN, aQ, Z, aJ, R, b9, a7, am, U, ad = -1, bb = I, bP = I, ac = q, bD = 0, b2 = 0, bo = [], L, a1 = I, X = new m.eventdispatcher(), aX = {
			play : "pause",
			mute : "unmute",
		...
		function aI(cg) {
			B.block(b3);
			if (cg.duration == Number.POSITIVE_INFINITY
					|| (!cg.duration && J.isSafari() && !d)) {
				bk.setText(b0.jwGetPlaylist()[b0.jwGetPlaylistIndex()].title
						|| "Live broadcast")
			} else {
				var ch;
				if (aR.elapsed) {
		...
		function bp(cg) {
			if (!a1) {
				var ci = b0.jwGetPlaylist()[cg.index].tracks, ch = I, ck = I;
				au();
				if (J.typeOf(ci) == x && !d) {
					for (var cj = 0; cj < ci.length; cj++) {
						if (!ch && ci[cj].file && ci[cj].kind
								&& ci[cj].kind.toLowerCase() == "thumbnails") {
							bN.load(ci[cj].file);
							ch = n
						}
						if (ci[cj].file && ci[cj].kind
								&& ci[cj].kind.toLowerCase() == "chapters") {
							ay(ci[cj].file);
							ck = n
						}
					}
		...
		function aD() {
			return (!!e.querySelector("#" + b0.id + " .jwplaylist") && !b0
					.jwGetFullscreen())
		}
		function bG() {
			...
			B.style(aC, cg);
			B(ag(".jwtext"), {
				font : bf.fontsize + "px/" + ah("background").height + "px "
						+ bf.font,
				color : bf.fontcolor,
		...
		function ak(ci, ch, cj, cn, ck) {
			var cm = bF(), co = ah(ci), cg = cn ? " left center" : " center", cp = br(co), cl;
			cm.className = "jw" + ci;
			cm.innerHTML = "&nbsp;";
			if (!co || !co.src) {
				return
		...
		function an(ch, co) {
			...
			cm.innerHTML = "&nbsp;";
			bE(cp, cm);
			var ci = ah(ch + "Button"), cn = ah(ch + "ButtonOver"), cq = ah(ch
					+ "ButtonOff");
			bg(ag(".jw" + ch + " button"), ci, cn, cq);
			var cl = aX[ch];
			if (cl) {
				bg(ag(".jw" + ch + ".jwtoggle button"), ah(cl + "Button"),
						ah(cl + "ButtonOver"))
			}
			aR[ch] = ck;
			return ck
		...
		function aN(cg, ch) {
			...
			if (aR[cg]) {
				aR[cg].className = "jw" + cg
						+ (ch ? " jwtoggle jwtoggling" : " jwtoggling");
				setTimeout(function() {
		...
		function bj(ch) {
			var ck = {}, cg = (ch == "alt") ? "elapsed" : ch, cj = ah(cg
					+ "Background");
			if (cj.src) {
				var ci = bF();
				ci.id = ae(ch);
		...
		function br(cg) {
			return cg ? parseInt(cg.width, 10) + "px "
					+ parseInt(cg.height, 10) + "px" : "0 0"
		}
		function af(ch) {
			var cg = ak(ch.name);
			if (!cg) {
		...
		function bx(cg) {
			...
			var ci = bF(), cl = cg == "volume", cj = cg
					+ (cg == "time" ? "Slider" : ""), co = cj + "Cap", ck = cl ? "Top"
					: "Left", cq = cl ? "Bottom" : "Right", cm = ak(co + ck, q,
					I, I, cl), cn = ak(co + cq, q, I, I, cl), ch = bH(cg, cl,
					ck, cq), cr = ah(co + ck), cp = ah(co + ck);
		...
		function bH(cz, ch, cl, cx) {
			var ci = bF(), cq = [ "Rail", "Buffer", "Progress" ], cm, ct;
			ci.className = "jwrail jwsmooth";
			for (var cs = 0; cs < cq.length; cs++) {
				ct = (cz == "time" ? "Slider" : "");
				var cr = cz + ct + cq[cs], ck = ak(cr, q, !ch, (cz
						.indexOf("volume") === 0), ch), cn = ak(
						cr + "Cap" + cl, q, I, I, ch), co = ak(cr + "Cap" + cx,
						q, I, I, ch), cj = ah(cr + "Cap" + cl), cg = ah(cr
						+ "Cap" + cx);
				if (ck) {
					var cp = bF();
					...
					cp.capSize = ch ? cj.height + cg.height : cj.width
							+ cg.width;
					B(ag("." + ck.className), {
						left : ch ? u : cj.width,
						right : ch ? u : cg.width,
		...
		function bU(cg) {
			...
			var cj = aR[ac].querySelector(".jwrail"), ck = J.bounds(cj), ci = cg.x
					/ ck.width;
			if (ci > 100) {
				ci = 100
			}
		...
		function aA(cg) {
			var ci = aR.time.querySelector(".jwrail"), cj = J.bounds(ci), ch = cg.x
					/ cj.width;
			if (ch > 100) {
				ch = 100
			}
		...
		function bM(cg) {
			...
			var ck = aR[ac].querySelector(".jwrail"), cl = J.bounds(ck), ch = ac, cj = aR[ch].vertical ? (cl.bottom - cg.pageY)
					/ cl.height
					: (cg.pageX - cl.left) / cl.width;
			if (cg.type == "mouseup") {
				if (ch == "time") {
					b0.jwSeekDrag(I)
				}
		...
		function V() {
			...
			var ci = "timeSliderCue", cg = ah(ci), ch = {
				"z-index" : 1
			};
			if (cg && cg.src) {
				ak(ci);
		...
		function a5(ck, cj) {
			if (ck.toString().search(/^[\d\.]+%?$/) >= 0) {
				var ch = ak("timeSliderCue"), ci = aR.timeSliderRail, cg = {
					position : ck,
					text : cj,
					element : ch
				};
		...
		function a6(ci, cg, cl, ch) {
			var cj = "volume" + (cg ? "" : "H"), ck = cg ? "vertical"
					: "horizontal";
			B(ag(".jw" + cj + ".jw" + ck),
					{
						width : ah(cj + "Rail", cg).width
								+ (cg ? 0 : (ah(cj + "Cap" + cl).width
										+ ah(cj + "RailCap" + cl).width
										+ ah(cj + "RailCap" + ch).width + ah(cj
										+ "Cap" + ch).width)),
						height : cg ? (ah(cj + "Cap" + cl).height
								+ ah(cj + "Rail").height
								+ ah(cj + "RailCap" + cl).height
								+ ah(cj + "RailCap" + ch).height + ah(cj
								+ "Cap" + ch).height) : u
					});
			ci.className += " jw" + ck
		}
		...
		bk.redraw = function(ch) {
			...
			B.unblock(b3);
			if (bk.visible) {
				var cj = ah("capLeft"), ci = ah("capRight"), cg = {
					left : Math.round(J.parseDimension(aY.left.offsetWidth)
							+ cj.width),
					right : Math.round(J.parseDimension(aY.right.offsetWidth)
							+ ci.width)
				};
		...
		function bZ(cj, cn) {
			...
			var ch = aR[cj].vertical, cm = cj + (cj === "time" ? "Slider" : ""), ck = 100
					* Math.min(Math.max(0, cn), 1) + "%", ci = aR[cm
					+ "Progress"], cg = aR[cm + "Thumb"], cl;
			if (ci) {
				cl = {};
				if (ch) {
					cl.height = ck;
	...
	B(A + " .jwcapRight," + A + " .jwtimeSliderCapRight," + A
			+ " .jwvolumeCapRight", {
		right : 0,
		position : a
	});
	...
	p(A, r);
	p(A + " button", r);
	p(A + " .jwtime .jwsmooth span", r
			+ ", width .25s linear, left .05s linear");
	p(A + " .jwtoggling", E)
})(window.jwplayer);
(function(d) {
	var c = d.html5, a = d.utils, f = d.events, b = f.state, h = d.playlist, e = true, g = false;
	c.controller = function(l, m) {
		var q = l, n = m, L = new f.eventdispatcher(q.id, q.config.debug), v = g, r = -1, D, M, Q = g, j, B = [];
		a.extend(this, L);
		function R() {
...
(function(d) {
	var g = d.html5, n = d.utils, o = d.events, p = o.state, k = n.css, l = n
			.isMobile(), m = document, a = ".jwdisplay", i = ".jwpreview", c = true, j = false, b = "absolute", h = "100%", e = "hidden", f = "opacity .25s, background-image .25s, color .25s";
	g.display = function(s, T) {
		var F = s.skin, ae, ah, O, K, u, H, Z, q = j, ai = {}, X = j, ad = j, aa = {}, r, Q, L, V, M, af = n
				.extend({
					showicons : c,
					bufferrotation : 45,
					bufferinterval : 100,
					...
					fontsize : 15,
					fontweight : ""
				}, F.getComponentSettings("display"), T), U = new o.eventdispatcher(), v, ac;
		n.extend(this, U);
		function ab() {
		...
		function ag(ak) {
			...
			var aj = n.bounds(ae.parentNode.querySelector(".jwcontrolbar")), ao = n
					.bounds(ae), an = {
		...
		function Y() {
			var aj = {
				font : af.fontweight + " " + af.fontsize + "px/"
						+ (parseInt(af.fontsize, 10) + 3)
		...
		function A(aj, ak) {
			...
			if (aj || ak) {
				L.setRotation(aj == "buffer" ? parseInt(af.bufferrotation, 10)
						: 0, parseInt(af.bufferinterval, 10));
				L.setIcon(aj);
				L.setText(ak)
			} else {
				L.hide()
		...
		function C() {
			if (ae.clientWidth * ae.clientHeight > 0) {
				n.stretch(s.jwGetStretching(), ah, ae.clientWidth,
						ae.clientHeight, H, Z)
			}
		}
		this.redraw = C;
...
(function(b) {
	...
	d.displayicon = function(n, H, s, B) {
		...
		function t() {
			...
			if (!h.isMobile()) {
				if (I.overSrc) {
					P["background-image"] = "url(" + O.overSrc + "), url("
		...
		function F(Q) {
			var R = v.getSkinElement("display", Q), P = v.getSkinElement(
					"display", Q + "Over");
			if (R) {
		...
		function r() {
			...
			var Q = "px " + e;
			var P = Math.ceil(Math.max(E.width, h.bounds(w).width - z.width
					- O.width));
			var R = {
				"background-size" : [ O.width + Q, P + Q, z.width + Q ]
						.join(", ")
			};
			if (w.parentNode) {
		...
		this.setRotation = G;
		function j(P) {
			return Math.floor(P.scrollHeight
					/ g.defaultView.getComputedStyle(P, null).lineHeight
							.replace("px", ""))
		}
...
(function(d) {
	var f = d.html5, k = d.utils, i = k.css, e = k.bounds, b = ".jwdock", h = ".jwdockbuttons", j = document, c = "none", a = "block", g = "100%";
	f.dock = function(w, G) {
		var z = w, y = {
			iconalpha : 0.75,
			iconalphaactive : 0.5,
			iconalphaover : 1,
			margin : 8
		}, r = k.extend({}, y, G), l = z.id + "_dock", s = z.skin, C = 0, o = {}, p = {}, t, D, H, A, B = this;
		function u() {
			B.visible = false;
			t = F("div", "jwdock");
		...
		function v() {
			var J = x("button"), K = x("buttonOver"), L = x("buttonActive");
			if (!J) {
				return
			}
			i(m(), {
		...
		function I(K) {
			var N = p[K], J, M = o[K], O, L = e(M.icon);
			N.offsetX(0);
			O = e(t);
			i("#" + N.element().id, {
		...
		B.addButton = function(K, S, O, L) {
			...
			var M = F("div", "divider", D), N = F("button", null, D), R = F(
					"div", null, N);
			R.id = l + "_" + L;
			R.innerHTML = "&nbsp;";
			i("#" + R.id, {
			...
			if (S) {
				var T = new f.overlay(R.id + "_tooltip", s, true), P = F("div");
				P.id = R.id + "_label";
				P.innerHTML = S;
				i("#" + P.id, {
					padding : 3
...
(function(b, f) {
	var a = b.html5, d = b.utils, e = b.events, c = e.state, g = b.playlist;
	a.instream = function(o, l, j, D) {
		...
		}, B, q, G, m, K, i, M, N, F, x, h = new e.eventdispatcher(), E, y, n = this, C = true, w = -1;
		...
		n.load = function(T, R) {
			...
			if (d.typeOf(T) == "object") {
				...
				y.setPlaylist([ T ]);
				P = d.extend(Q, R);
				B = new a.adskipbutton(o, V.height - (U.y + U.height) + 10,
						P.skipMessage, P.skipText);
		...
		n.jwInstreamDestroy = function(R) {
			...
			if (x) {
				if (q && q.parentElement) {
					q.parentElement
							.removeEventListener("click", x.clickHandler)
				}
...
(function(c) {
	var m = c.utils, h = c.html5, l = m.css, o = c.events.state, i = undefined, j = "free", f = "pro", g = "premium", n = "ads", e = "open", p = "http://www.longtailvideo.com/jwpabout/?a=l&v=", a = "visible", d = "hidden", k = ".jwlogo";
	var b = h.logo = function(x, y) {
		var D = x, E = D.id + "_logo", u, r, v = b.defaults, C = false;
		function w() {
			B();
			s()
		}
		...
		function s() {
			...
			var F = (/(\w+)-(\w+)/).exec(u.position), G = {}, I = u.margin;
			if (F.length == 3) {
				G[F[1]] = I;
				G[F[2]] = I
			} else {
			...
			l(q(), G);
			//r.src = (u.prefix ? u.prefix : "") + u.file;  文件
			if (!m.isMobile()) {
				//r.onclick = A   点击链接
			} else {
				var H = new m.touch(r);
...
(function(c) {
	var f = c.html5, j = c.utils, i = j.css, h = "jwmenu", d = "jwoption", g, a = "#ffffff", b = "#cccccc";
	f.menu = function(l, m, z, s) {
		var w = m, n = s, p = new f.overlay(w + "_overlay", z), q = j.extend({
			fontcase : g,
			fontcolor : b,
			fontsize : 11,
			fontweight : g,
			activecolor : a,
			overcolor : a
		}, z.getComponentSettings("tooltip")), o, y = [];
		function v() {
			o = t(h);
			o.id = w;
			var E = r("menuTop" + l), C = r("menuOption"), B = r("menuOptionOver"), D = r("menuOptionActive");
			if (E && E.image) {
				var F = new Image();
				F.src = E.src;
				F.width = E.width;
			...
			if (C) {
				...
				i(A, j.extend(x(C), {
					...
					font : q.fontweight + " " + q.fontsize
							+ "px Arial,Helvetica,sans-serif",
					"line-height" : C.height,
					"text-transform" : (q.fontcase == "upper") ? "uppercase"
							: g
				}));
...
(function(b) {
	...
	b.model = function(i, h) {
		var o = this, k, q, r = a.getCookies(), g = {
			controlbar : {},
			display : {}
		}, m = {
			autostart : f,
		...
		function j(s) {
			var z = (l[s.type] ? l[s.type].split(",") : []), w, y;
			if (z.length > 0) {
				for (w = 0; w < z.length; w++) {
					var u = z[w], v = u.split("->"), x = v[0], t = v[1] ? v[1]
							: x;
					if (o[t] != s[x]) {
						o[t] = s[x];
						y = true
...
(function(j) {
	var e = j.html5, q = j.utils, m = q.css, r = q.transitionStyle, c = "relative", d = "absolute", g = "hidden", i = "100%", p = "opacity .25s, visibility .25s", k = ".jwoverlay", a = "jwcontents", o = "top", f = "bottom", h = "right", l = "left", s = "#ffffff", t, b = document, n = {
		fontcase : t,
		fontcolor : s,
		fontsize : 12,
		fontweight : t,
	...
	e.overlay = function(G, I, E) {
		var J = this, w = G, B = I, K = E, C, N, H, u, v = q.extend({}, n, B
				.getComponentSettings("tooltip")), y = {};
		function D() {
			C = L(k.replace(".", ""));
			C.id = w;
			...
			m.style(C, {
				padding : (y.top + 1) + "px " + y.right + "px "
						+ (y.bottom + 1) + "px " + y.left + "px"
			});
			J.showing = false
		}
		function z(O) {
		...
		function M(U, T) {
			...
			var Q = x("cap" + U + T, "jwborder jw" + U + (T ? T : "")), O = Q[0], R = Q[1], P = q
					.extend(
							A(R),
							{
								width : (U == l || T == l || U == h || T == h) ? R.width
										: t,
								height : (U == o || T == o || U == f || T == f) ? R.height
										: t
							});
...
(function(g) {
	var b = "#FFFFFF", d = "#CCCCCC", k = "#333333", h = "#999999", j = "normal", f = {
		size : 180,
		backgroundcolor : k,
		fontcolor : h,
		overcolor : d,
		...
		fontsize : 11,
		titlesize : 13
	}, q = jwplayer.events, o = jwplayer.utils, l = o.css, m = o.isMobile(), p = ".jwplaylist", n = document, a = "absolute", c = "relative", e = "hidden", i = "100%";
	g.playlistcomponent = function(K, V) {
		var O = K, D = O.skin, t = o.extend({}, f, O.skin
				.getComponentSettings("playlist"), V), P, E, r, v, C = -1, F, u, x = -1, w = 76, y = {
...
(function(j) {
	var a = jwplayer.events, t = jwplayer.utils, o = t.touchEvents, n = t.css, b = "jwslider", q = "jwslidertop", h = "jwsliderbottom", f = "jwrail", r = "jwrailtop", p = "jwrailback", m = "jwrailbottom", c = "jwthumb", v = "jwthumbtop", i = "jwthumbback", u = "jwthumbbottom", e = document, s = window, w = undefined, g = "absolute", k = "hidden", l = "100%";
	j.playlistslider = function(V, N, J, aa) {
		var O = N, X = V, ab = aa, ai, E, af, S, aj = 0, W, ag, C = t
				.isMobile(), al, am = true, F, R, ae, z, ad, x, G, Q, U, ah, L;
		this.element = function() {
			return ai
		};
		...
		function T(an) {
			...
			if (ab) {
				ab.style.width = am ? ab.parentElement.clientWidth - ae.width
						+ "px" : ""
			}
		}
		var M = this.thumbPosition = function(an) {
			...
			if (aa) {
				aa.style.top = Math.min(0, ai.clientHeight - aa.scrollHeight)
						* aj + "px"
			}
		};
		function D(an) {
		...
		function ac(an) {
			if (S || an.type == "click") {
				var at = t.bounds(E), ap = af.clientHeight / 2, ao = at.height
						- ap, ar = an.pageY - at.top, aq = (ar - ap)
						/ (ao - ap);
				M(aq)
			}
		}
...
(function(e) {
	var k = jwplayer.utils, i = k.css, a = "About JW Player ", l = "http://www.longtailvideo.com/jwpabout/?a=r&v=", j = document, h = ".jwclick", g = h
			+ "_item", f = "100%", b = "none", d = "5px 5px 7px rgba(0,0,0,.10), 0px 1px 0px rgba(255,255,255,.3) inset", c = "#FFF";
	e.rightclick = function(q, o) {
		...
		function n(A) {
			...
			var C = A.target != null ? A.target : A.srcElement, z = k.bounds(p), B = k
					.bounds(C);
			x.style.display = b;
...
(function(f) {
	var h = jwplayer, k = h.utils, l = h.events, a = h.playlist, i = 1, e = 2, d = 3, j = 4, c = 5, b = 6, g = 7;
	f.setup = function(s, H, I) {
		var L = s, p = H, F = I, u = {}, C = {}, A, z = new l.eventdispatcher(), v = false, w = [];
		function t() {
			r(i, o);
			r(e, Q, i);
			r(d, y, i);
...
(function(b) {
	var a = jwplayer.utils, d = a.foreach, c = "Skin formatting error";
	b.skinloader = function(g, l, i) {
		var j = {}, n = l, v = i, s = true, w, x = g, h = false, u, z = jwplayer.utils
				.isMobile() ? 1 : 1, y = 1;
		function B() {
		...
		function A(H) {
			var G = m(H, "skin")[0], P = m(G, "component"), ab = G
					.getAttribute("target"), K = parseFloat(G
			...
			for (var S = 0; S < P.length; S++) {
				var N = k(P[S].getAttribute("name")), M = {
					settings : {},
					elements : {},
					layout : {}
		...
		function p(H, G) {
			G = k(G);
			var F = new Image(), C = k(H.getAttribute("name")), E = H
					.getAttribute("src"), J;
...
(function(c) {
	var a = c.utils, e = c.events, b = e.state, d = true, f = false;
	c.html5.video = function(ad) {
		var v = a.isIE(), Y = {
			...
			volumechange : o,
			waiting : A
		}, K, S, B, aj, t, ai, R, Z, M = f, g = b.IDLE, T, s = -1, P = -1, V = new e.eventdispatcher(), y = f, O, J = -1, Q = a
				.isAndroid(f, d), j = a.isIOS(7), k = this, n = [], L = 0, i = false, ak = f, D = f;
		a.extend(k, V);
		function af(al) {
			if (!al) {
		...
		function ac(al) {
			...
			if (B.paused) {
				if (B.currentTime == B.duration && B.duration > 3) {
				} else {
					h()
				}
			} else {
		...
		function G() {
			...
			var an = a.getCookies(), al = an.qualityLabel;
			if (al) {
				...
				a.foreach(al, function(ap, aq) {
					if (aq.data) {
						...
						a.foreach(aq.data, function(ar, at) {
							if (ar % 2 == 1) {
								ao.addCue(new TextTrackCue(at.begin,
										aq.data[parseInt(ar) + 1].begin,
										at.text))
							}
						});
						n.push(ao);
...
(function(r) {
	var n = r.html5, A = r.utils, c = r.events, i = c.state, u = A.css, w = A.bounds, z = A
			.isMobile(), g = A.isIPad(), D = A.isIPod(), k = A.isAndroid(), E = A
			.isIOS(), j = document, s = "jwplayer", e = "aspectMode", d = "."
			+ s + ".jwfullscreen", t = "jwmain", C = "jwinstream", B = "jwvideo", f = "jwcontrols", b = "jwaspect", h = "jwplaylistcontainer", m = true, v = false, y = "opacity .25s ease", q = "100%", l = "absolute", x = " !important", o = "hidden", a = "none", p = "block";
	n.view = function(U, Z) {
		var aO = U, H = Z, aZ, a4, aR, ab, R, aU = -1, aH = z ? 4000 : 2000, aj, ax, av, aa, af, aG, K, X, aE = v, aW, ag, aF, M, aB = A
				.extend({}, H.componentConfig("logo")), aT, O, aK, ar = v, at = v, az, an, aq, a2 = -1, P = v, au, Y = new c.eventdispatcher();
		A.extend(this, Y);
		function aP() {
		...
		function aN() {
			var ba = w(aZ), bc = Math.round(ba.width), bb = Math
					.round(ba.height);
		...
		this.setup = function(bc) {
			...
			var ba = A.exists(H.aspectratio) ? parseFloat(H.aspectratio) : 100, bb = H.playlistsize;
		...
		function T() {
			var ba = H.height, bc = H.componentConfig("controlbar"), bb = H
		...
		function S(bb, bi, bf) {
			var be = aZ.className, bj, bd, bh, bc, bg, ba = aO.id + "_view";
			u.block(ba);
			bf = !!bf;
			if (bf) {
			...
			if (M) {
				M.offset(aW && M.position().indexOf("bottom") >= 0 ? aW
						.height()
						+ aW.margin() : 0);
				setTimeout(function() {
					if (aF) {
						aF
								.offset(M.position() == "top-left" ? M
										.element().clientWidth
										+ M.margin() : 0)
					}
				}, 500)
		...
		function ao(bb, ba) {
			if (aj) {
				...
				var bc = A.stretch(H.stretching, aj, bb, ba, aj.videoWidth,
						aj.videoHeight);
				if (bc) {
					clearTimeout(a2);
		...
		function W() {
			var ba = j.mozFullScreenElement || j.webkitCurrentFullScreenElement
					|| j.msFullscreenElement || aj.webkitDisplayingFullscreen;
		...
		this.getSafeRegion = function() {
			...
			aW.showTemp();
			aF.showTemp();
			var bc = w(a4), bd = bc.top, ba = aE ? w(j.getElementById(aO.id
					+ "_instream_controlbar")) : w(aW.element()), bh = aE ? false
					: (aF.numButtons() > 0), bb = (M.position().indexOf("top") === 0), bg, be = w(M
					.element());
			if (bh) {
				bg = w(aF.element());
